...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    img2 = np.array(Image.fromarray(img).resize((int(len(img) * 0.5), int(len(img[0]) * 0.5)), resample = Image.BICUBIC))
    # img2 = np.array(Image.fromarray(img).resize((434, 614), resample = Image.BICUBIC))
